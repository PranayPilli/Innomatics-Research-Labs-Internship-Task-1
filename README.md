# Innomatics Research Labs – Internship Task 1

This repository contains my submission for Task 1 of the Advanced Generative AI Internship at Innomatics Research Labs.

The assignment focuses on building core Python logic through practical problems including:

1) User authentication check  
2) Pass/Fail result analysis  
3) Data cleaning and standardization  
4) Message length validation  
5) Error log detection  

The implementation uses fundamental Python concepts such as conditional statements, loops, lists, and string methods.

# Innomatics Research Labs – Internship Task 2

This repository contains my submission for Logic Building Task 2 of the Advanced Generative AI Internship at Innomatics Research Labs.

The assignment focuses on strengthening Python fundamentals through practical problems including:

1) Unique words identification in a sentence  
2) Highest salary detection from employee data  
3) Finding maximum and minimum values  
4) Counting products above a price threshold  
5) Calculating attendance percentage  
6) Removing duplicate phone numbers  
7) Counting character frequency  
8) Converting list to tuple  
9) Checking key existence in a dictionary  
10) Calculating average marks  

The implementation uses core Python concepts such as lists, dictionaries, sets, loops, conditional statements, and basic mathematical operations.


All solutions are implemented using clean, structured, and well-commented Python code as per the internship guidelines.

# Innomatics Research Labs – Internship Task 3

This repository contains my submission for Data Processing and Analysis Task 3 of the Advanced Generative AI Internship at Innomatics Research Labs.

The assignment focuses on applying Python programming concepts to real-world data processing and analysis problems including:

1) Employee performance bonus eligibility detection
2) Search query keyword frequency analysis
3) Sensor data validation using even number filtering
4) Email domain usage percentage calculation
5) Sales spike detection based on average comparison
6) Duplicate user ID detection and occurrence counting

The implementation uses core Python concepts such as dictionaries, lists, tuples, loops, conditional statements, string manipulation, enumeration, and basic statistical calculations.

All solutions are implemented using clean, structured, and well-commented Python code as per the internship guidelines.

# Innomatics Research Labs – Internship Task 4

This repository contains my submission for Task 4 – Functions with Loops of the Advanced Generative AI Internship at Innomatics Research Labs.

The assignment focuses on implementing real-world logic using Python functions and loop structures through practical problems including:

1) Mobile recharge validation system
2) Inventory reorder alert detection
3) Student result processing and average calculation
4) Cab fare estimation with peak hour adjustment
5) Employee attendance eligibility checking
6) Password strength validation

The implementation uses core Python concepts such as user-defined functions, while loops, for loops, conditional statements, list operations, string handling, and basic mathematical calculations.
